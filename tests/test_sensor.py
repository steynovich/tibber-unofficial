...
def test_sensor_state_current_day():
    ...
    sensor_desc = next(
        s for s in SENSOR_DESCRIPTIONS if s.key == "current_day_homevolt"
    )
    ...
    # Test EV sensor
    sensor_desc = next(s for s in SENSOR_DESCRIPTIONS if s.key == "current_day_ev")
    sensor = GridRewardsSensor(coordinator, mock_config, sensor_desc)
    assert sensor.native_value == 1.0

    # Test Total sensor
    sensor_desc = next(s for s in SENSOR_DESCRIPTIONS if s.key == "current_day_total")
    sensor = GridRewardsSensor(coordinator, mock_config, sensor_desc)
...
def test_sensor_state_current_month():
    ...
    sensor_desc = next(
        s for s in SENSOR_DESCRIPTIONS if s.key == "current_month_homevolt"
    )
    sensor = GridRewardsSensor(coordinator, mock_config, sensor_desc)
...
def test_sensor_state_previous_month():
    ...
    sensor_desc = next(
        s for s in SENSOR_DESCRIPTIONS if s.key == "previous_month_total"
    )
    sensor = GridRewardsSensor(coordinator, mock_config, sensor_desc)
...
def test_sensor_state_current_year():
    ...
    sensor_desc = next(s for s in SENSOR_DESCRIPTIONS if s.key == "current_year_total")
    sensor = GridRewardsSensor(coordinator, mock_config, sensor_desc)
